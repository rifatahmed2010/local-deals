...
//            ->orderBy('id', 'desc')         // Order by ID in descending order
    ...
    public function searchDealListA(Request $request)
    {
        ...
        if($request->dealCategory && $request->searchKey){
            ...
            $deals = Deal::where('deal_category',$request->dealCategory)
                ->where(function ($query) use ($searchKey) {
                    $query->where('description', 'like', "%$searchKey%")
                        ->orWhere('deal_title', 'like', "%$searchKey%")
                        ->orWhere('business_name', 'like', "%$searchKey%");
                })
            ...
            $searchKey = $request->searchKey;
            $deals = Deal::where(function ($query) use ($searchKey) {
                $query->where('description', 'like', "%$searchKey%")
                    ->orWhere('deal_title', 'like', "%$searchKey%")
                    ->orWhere('business_name', 'like', "%$searchKey%");
                })
    ...
    public function getDealById(Request $request) {
        ...
        $deal = Deal::find($request->id);
        if($deal){
            $this->checkFavouriteForSingleDeal($deal);
    ...
    public function homeAuth(Request $request,$user)
    {
        ...
        $top10DealsIds = DB::table('claim_deals as a')
            ...
            ->whereDate('a.created_at', '>=', Carbon::now()->subDays(7)->toDateString())
    ...
    public function searchDealsByTag(Request $request){
        ...
        if ($token) {
            ...
            if ($accessToken) {
                $user = $accessToken->tokenable;
                return $this->searchAuth($request,$user);
            }
        }else{
    ...
    public function searchAuth(Request $request,$user){
        ...
        if($tagName=='Biggest Savings'){
            $deals = Deal::where('is_active',1)
                ->where('expired_date', '>', $today)
                ->whereNotIn('id', $claimedLimitedIds)
                ->when($university, function ($query, $university) {
                    return $query->where('university_name', $university);
                })
                ->skip($skip)
                ->take($size)
                ->orderByDesc('total_saving')->limit(10)->get();
        ...
        $deals = $this->excludeDeals($user,$deals);
    ...
    public function checkFavourite($deals){
        $user = auth('sanctum')->user();

        // Convert to a collection for `contains()` method
        $favouriteDealIds = collect(
            FavouriteDeal::where('user_id', $user->id)
                ->where('is_active', 1)
                ->pluck('deal_id')
                ->toArray()
        );

        $deals = $deals->map(function ($deal) use ($favouriteDealIds) {
            $deal->is_favourite = $favouriteDealIds->contains($deal->id) ? 1 : 0;
            return $deal;
        });

        return $deals; // Ensure the modified collection is returned
    }

    public function checkFavouriteForSingleDeal($deal) {
        $user = auth('sanctum')->user();

        // Get favorite deal IDs for the user
        $favouriteDealIds = FavouriteDeal::where('user_id', $user->id)
            ->where('is_active', 1)
            ->pluck('deal_id')
            ->toArray();

        // Check if the single deal is in favorites
        $deal->is_favourite = in_array($deal->id, $favouriteDealIds) ? 1 : 0;
